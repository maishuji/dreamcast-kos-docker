...
`--kos-ports-branch` is passed unchanged to the existing Docker build argument `snapshot_kosports`. For a direct build, use `docker build --build-arg snapshot_kosports=feature/my-branch -t yourname/dc-kos-image:custom ./src/dcdocker/assets/kos-ready/`. Direct builds temporarily default to `fix/make-build-all-succeed` while [the upstream fix](https://github.com/maishuji/kos-ports/tree/fix/make-build-all-succeed) is under review; pass `--build-arg snapshot_kosports=master` to override it.
